...
> **Note:** If the servo vibrates without moving, use VIN (5V) instead of the 3.3V pin. A 100µF capacitor on VIN can stabilize the supply.
...
- Camera pan slider (0°–180°) with center reset button
...
The input pipeline pre-processes each frame through **image tessellation** — dividing the frame into a grid of 150×150×3 patches — which improves detection of small, localized flame regions that would otherwise be missed at full resolution.
...
| ⬛ `background` | Empty background, no object of interest | — |
...
- ~3,000 annotated bounding boxes per class
- 0–10% background-only images to minimize the false positive rate in production
...
Evaluated on a 3,000-image held-out test set:
...
🔗 **Live Edge Impulse Project:** [studio.edgeimpulse.com/public/914533/live](https://studio.edgeimpulse.com/public/914533/live)
...
- Replace ESP32-CAM with **Raspberry Pi 4B** + wide-angle camera for HD inference
